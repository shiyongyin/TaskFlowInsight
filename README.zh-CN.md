...
让 Java 21 应用中的业务执行过程与对象变化可解释

[English](README.md)

</div>

TaskFlowInsight 帮助 Java 团队看清一次业务处理“经历了什么、哪里出了问题、数据发生了什么变化”，并把这些信息交给开发、测试和运维共同使用。

它以组件方式嵌入应用，本地记录与导出不要求另行部署 TFI 服务。生产环境中的持久化、查询和告警仍需接入应用已有设施或外部平台。

面向业务应用的 TFI 完整功能线是当前默认选择。Kernel 是独立的发布候选（RC）试验线，接口仍可能调整。

[项目是什么](#当前项目是什么) · [解决的问题](#解决什么问题) · [应用场景](#应用场景) · [使用案例](#使用案例)

[版本选择](#tfi-完整版与-kernel-rc-版怎么选) · [内容构成](#内容构成) · [维护成本](#维护成本) · [快速开始](#快速开始)

## 当前项目是什么

TaskFlowInsight 是一组嵌入 Java 21 应用的开源组件，用于记录业务执行过程，并解释两个对象状态之间的变化。它不是需要单独部署的服务，也不提供独立的运维界面。

业务代码通过 API 或注解标出值得关注的处理过程，并显式调用比较能力。

TFI 分别生成可检查的流程记录和对象比较结果：哪些步骤被记录、每一步的状态和耗时、对象前后有哪些变化，以及比较结果是否完整。

它的目标不是替代日志、Trace 或 APM，而是补充这些基础设施默认难以表达或可靠还原的业务语义。它也不是工作流引擎，不负责调度业务流程。

当前源码版本为 `4.0.0-SNAPSHOT`，尚未公开发布。新项目优先使用 TFI 完整功能线；Kernel 只用于有明确目标、负责人和回滚方案的受控 RC 试用。

## 解决什么问题

| 用户想知道什么 | 现有方式的常见困难 | TaskFlowInsight 提供的结果 |
|---|---|---|
| 一次业务请求到底走了哪些步骤 | 日志分散在多个方法和线程中，需要人工拼接 | 一份按已记录业务步骤组织的执行记录 |
| 哪一步失败或明显变慢 | 只有异常栈或总耗时，缺少业务阶段上下文 | 每一步的状态、耗时和业务消息 |
| 更新前后哪些数据发生了变化 | 手写字段日志容易遗漏，复杂集合难以说明 | 带路径的对象差异，以及比较是否完整和不完整原因 |
| 一次回归是否走了预期分支 | 测试通常只断言最终返回值 | 可用于测试断言的执行路径和结构化结果 |
| 开发、测试和运维如何看到同一事实 | 各角色使用不同日志口径，沟通成本高 | 可读报告与机器格式共享同一份业务事实 |

## 应用场景

| 业务场景 | 需要回答的问题 | 推荐选择 |
|---|---|---|
| 订单创建与履约 | 参数校验、库存、价格、支付、落库分别是否成功，慢在哪里 | TFI 完整功能线 |
| 审批与规则决策 | 实际经过哪些节点，记录了哪些分支依据，在哪一步拒绝 | TFI 完整功能线 |
| 计费、结算与对账 | 本次记录覆盖了哪些计算步骤，账单或费用项发生了哪些变化 | 完整线 Flow + Compare |
| 库存与状态流转 | 扣减、释放、回补是否被记录，状态前后发生了什么变化 | 完整线 Flow + Compare |
| 配置与价格变更 | 哪些字段或集合项发生变化，比较结果是否完整 | 按需使用完整线 Compare |
| 自动化回归测试 | 业务结果正确时，已记录的内部路径是否符合测试断言 | 按需使用完整线 Flow |
| 平台或 SDK 统一采集 | 只需采集少量事实，并限制单次记录的数量与体积 | 受控试用 Kernel RC |

Kernel 中的“有界”表示每个会话和单条事实的阶段数、属性数与输出体积都有硬上限，用于控制宿主资源。

超出限制的当前内容会被拒绝；会话字节预算耗尽后，后续追加会停止，结果也会标记为不完整。这不表示业务流程本身更简单。

## 使用案例

### 案例一：定位订单提交失败

团队把参数校验、库存、计价、支付和落库标记为业务步骤。

如果业务代码正确标记失败并记录原因，某次订单失败后，记录可以显示已经执行的步骤、失败阶段、各阶段耗时和相关业务消息。

开发不必先搜索整条日志链；测试可以核对失败路径是否符合预期；运维可以结合已记录的状态、耗时和原因辅助判断。

TFI 不会自动识别业务拒绝，也不会替应用定义“慢”的阈值。

### 案例二：审核价格配置发布

发布前比较新旧价格配置，结果列出发生变化的字段、列表项和规则，并明确这次比较是否完整。

发布人确认实际改动，审核人据此决定是否继续人工检查。

TFI 只说明观察到的变化，不会自动证明这些变化已经提交，也不会替业务系统作出发布或审批决定。

### 案例三：平台组件试用 Kernel

平台已经具备统一采集、权限、存储和留存能力，只希望各服务提交固定、大小受控的事实。

平台团队先定义记录约定、Sink、脱敏、监控和回滚，再由少量服务受控试用 Kernel。

如果这些平台能力不存在，试用方必须自行承担相应建设和维护成本。

对普通业务应用，完整功能线可以减少流程记录、对象比较和导出能力的自建，但同样不提供托管式持久化、查询或告警平台。

## TFI 完整版与 Kernel RC 版怎么选

普通业务应用默认选择 TFI 完整功能线。已有平台闭环时，或单个真实服务有一个现有工具难以回答的问题，并愿意承担缺失能力时，才评估 Kernel RC。

本文所说的“完整版”指 TFI 完整功能线，不等于必须使用完整聚合包。新应用仍然优先按需选择模块，确实需要大部分能力时再使用聚合包。

| 选择维度 | TFI 完整功能线 | Kernel RC 线 |
|---|---|---|
| 产品定位 | 可直接为业务应用提供流程解释与对象变化能力 | 为平台、SDK 或基础组件提供有界事实记录底座 |
| 主要使用者 | 业务开发、测试、运维和现有 TFI 用户 | 平台、基础设施，或边界明确的受控服务团队 |
| 开箱获得 | 流程记录、对象比较、纯 Java 接入，以及可选 Spring 和 Ops | 有界记录运行时；Compare 与 Spring 组合仍是内部预览，不作为当前试用入口 |
| 接入方仍需负责 | 数据边界、敏感信息，以及外部持久化、查询、告警、授权和留存 | Sink、字段约定、脱敏、传输、存储、查询、监控、留存和回滚 |
| 当前成熟度 | 当前推荐线，但 4.0 仍是未发布源码快照 | RC，API 未冻结，真实服务价值仍待验证 |
| 采用建议 | 新应用默认；按需模块优先，确实使用多数能力时再选聚合包 | 仅用于受控试用，不作为普通业务应用默认入口 |

### 完整版适合什么场景

- 希望直接解释订单、审批、计费、库存等业务流程。
- 需要对象比较、Spring 注解或运维集成，而不想自行建设记录和比较基础能力。
- 团队希望从按需模块开始，并保留后续增加能力的空间。
- 现有代码已经使用 `TFI`、`TfiFlow` 或完整线 Compare。

### RC 版适合什么场景

- 平台或 SDK 已经有统一采集、存储、权限、监控和留存能力；或者单个服务有一个边界明确、现有工具难以回答的问题。
- 只需要记录事先约定的少量事实，并接受达到数量或体积上限时结果可能不完整。
- 试用问题、负责人、数据边界、停用开关和回滚验证都已经明确。
- 团队接受 RC API 和 Schema 仍可能调整。

Kernel 不是 TFI 完整功能线的轻量版，也不是聚合包的低配套餐。只想减少依赖时，应选择完整功能线的按需模块；职责更窄不代表总使用成本更低。

## 内容构成

| 内容 | 提供什么 | 当前建议 |
|---|---|---|
| TFI 完整功能线 | 业务流程记录、对象比较、纯 Java API、可选 Spring 集成与可选 Ops 实现 | 普通业务应用的推荐入口 |
| 完整聚合包 | 用一个坐标引入完整线全部能力和统一 `TFI` 门面 | 存量用户或确实使用多数能力时选择 |
| 完整线按需模块 | 只引入实际使用的 Flow、Compare、Spring 或 Ops | 新应用默认推荐 |
| Kernel Core | 记录数量和体积受限的阶段与事实，由宿主负责后续数据闭环 | 平台或边界明确的服务受控试用 |
| Kernel + Compare | 把独立比较结果映射为有界记录 | 已选择 Kernel 后才评估的内部预览 |
| 示例、文档与质量门禁 | 提供可运行示例、模块设计说明、测试与 CI 验证证据 | 用于学习、评估和维护，不作为业务依赖 |

## 维护成本

维护成本不仅是依赖数量，还包括配置、测试、安全、数据治理、升级和跨团队协作。

Kernel 边界之外的职责仍由接入方承担，不会自动消失。

下表是仓库内四种选择的相对、定性责任评估，不代表工程工时、预算或运行性能。Kernel 尚未完成真实服务试用，实际总成本仍待验证。

| 方案 | 首次接入 | 长期维护 | 团队需要持续承担什么 |
|---|---|---|---|
| 完整线按需模块 | 中 | 低到中 | 管理已选能力、业务埋点、升级回归和数据边界 |
| 完整聚合包 | 低 | 中 | 管理更宽的依赖、自动配置范围和组合回归 |
| Kernel Core | 高 | 高；有统一平台时可分摊 | 自建并维护 Sink、约定、安全、监控、留存与回滚 |
| Kernel + Compare | 高 | 最高 | 在 Kernel 成本上再承担双模型、脱敏、组合测试和版本风险 |

认知成本也不同：完整版按需接入时，初始学习主要围绕业务 API；扩展到 Spring、Ops、输出与安全后，仍需理解对应链路。

Kernel 名词更少，却要求团队理解更多外部系统、失败边界和责任分工。

如果平台团队能统一维护 Kernel 规范，多个服务可以共享这部分成本；单个服务自行接入时，成本会直接落在该服务团队。当前仍需真实试用证明 Kernel 能降低组织总成本。

## 快速开始

当前版本是尚未公开发布的 `4.0.0-SNAPSHOT`，需要先从源码安装到本地 Maven 仓库：
...
新应用默认按需引入能力。下面使用纯 Java `tfi-flow-core` 建立最小流程记录闭环：
...
流程必须在 `endSession()` 前导出。生产接入还应先确定记录边界、敏感数据规则和输出去向。

上面代码的输出结构如下，已省略动态 ID、时间和耗时字段：
...
导出发生在会话结束前，因此示例中的会话状态仍为 `RUNNING`，已经关闭的业务步骤为 `COMPLETED`。

要直接运行 Spring Boot 示例：
...
继续接入可查看[完整聚合包](#tfi-完整版聚合包接入)、[Spring Flow](#spring-flow)、[对象比较](#只用-compare)或 [Kernel RC](#kernel-rc-线)。
...
<details>
<summary><strong>查看完整选型、使用成本与认知成本分析</strong></summary>
...
<details>
<summary><strong>查看模块关系与完整技术接入参考</strong></summary>
