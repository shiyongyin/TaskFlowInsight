...
public class ManagedThreadContext implements AutoCloseable {
    ...
    public static ManagedThreadContext create(String rootTaskName) {
        ...
        if (existing != null && !existing.closed) {
            logger.debug("Creating new context while existing context is active. Existing context will be closed: {}", existing.contextId);
            existing.close();
        }
    ...
    public void close() {
        ...
        if (currentSession != null && currentSession.isActive()) {
            ...
            try {
                ...
                // 忽略清理异常，避免影响close流程
                logger.debug("Failed to clear tracking during close: {}", t.getMessage());
            }
