...
public final class Session {
    ...
    public synchronized Session activate() {
        if (!isActive()) {
            throw new IllegalStateException("Cannot activate session that is not running. Current status: " + status.get());
        }
    ...
    public synchronized void complete() {
        if (!status.compareAndSet(SessionStatus.RUNNING, SessionStatus.COMPLETED)) {
            throw new IllegalStateException("Cannot complete session that is not running. Current status: " + status.get());
        }
    ...
    public synchronized void error() {
        if (!status.compareAndSet(SessionStatus.RUNNING, SessionStatus.ERROR)) {
            throw new IllegalStateException("Cannot error session that is not running. Current status: " + status.get());
        }
