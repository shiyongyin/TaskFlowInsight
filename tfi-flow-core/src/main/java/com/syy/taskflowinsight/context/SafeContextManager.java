...
public final class SafeContextManager {
     ...
     * 配置上下文管理器参数
     *
     * <p>此方法用于直接配置上下文参数，不依赖 Spring 配置类。
     * Spring 环境下的配置由 tfi-flow-spring-starter 模块处理。
     *
     * @param contextTimeoutMillis 上下文超时时间（毫秒）
     * @param leakDetectionEnabled 是否启用泄漏检测
     * @param leakDetectionIntervalMillis 泄漏检测间隔（毫秒）
     */
    public void configure(long contextTimeoutMillis, boolean leakDetectionEnabled, 
                         long leakDetectionIntervalMillis) {
        this.contextTimeoutMillis = contextTimeoutMillis;
        this.leakDetectionEnabled = leakDetectionEnabled;
        this.leakDetectionIntervalMillis = leakDetectionIntervalMillis;
        
        logger.info("SafeContextManager configured: " +
            "contextTimeout={}ms, leakDetectionEnabled={}, leakDetectionInterval={}ms",
            this.contextTimeoutMillis, this.leakDetectionEnabled, this.leakDetectionIntervalMillis);
     ...
     * 启用或禁用泄漏检测。
     *
     * @param enabled true 启用检测并启动调度器，false 停止调度器
     */
     ...
     * 设置泄漏检测间隔。
     *
     * @param intervalMillis 检测间隔毫秒数；若已启用检测则重启调度器
     */
