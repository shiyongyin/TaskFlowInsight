...
public class ConfigurationResolverImpl implements ConfigurationResolver {
    ...
    public ConfigurationResolverImpl(Environment environment, ConfigMigrationMapper migrationMapper) {
        // BUG-FIX: 支持非 Spring 环境调用（传入 null 时回退到 StandardEnvironment）
        this.environment = environment != null ? environment : new org.springframework.core.env.StandardEnvironment();
        this.migrationMapper = migrationMapper != null ? migrationMapper : new ConfigMigrationMapper();
        // 使用 this.environment 而非原始参数，避免 environment=null 时 NPE
        this.resolverEnabled = this.environment.getProperty(
            ConfigDefaults.Keys.RESOLVER_ENABLED, 
            Boolean.class, 
            true
        );
        // 兼容两种环境变量配置键名，并支持短名映射
        Boolean envEnabled = this.environment.getProperty(ConfigDefaults.Keys.ENV_ENABLED, Boolean.class);
        if (envEnabled == null) {
            envEnabled = this.environment.getProperty(ConfigDefaults.Keys.ENV_VARIABLES_ENABLED, Boolean.class);
        }
