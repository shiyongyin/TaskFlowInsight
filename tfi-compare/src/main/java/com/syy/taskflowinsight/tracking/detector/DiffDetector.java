...
public final class DiffDetector {
    ...
    // 精度比较策略（可选增强，默认关闭）
    // Thread-safety: volatile 保证多线程可见性（配置在初始化阶段设置，运行期读取）
    private static volatile boolean precisionCompareEnabled = false;
    ...
    private static volatile PrecisionController precisionController;
    private static volatile TfiDateTimeFormatter dateTimeFormatter;
    private static volatile PrecisionMetrics precisionMetrics;
    // Thread-safety: volatile 防止并发调用时读取脏值
    private static volatile Class<?> currentObjectClass; // 用于字段查找
    ...
    private static volatile boolean enhancedDeduplicationEnabled = true;
