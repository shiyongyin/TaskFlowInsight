...
public class PathArbiter {
     ...
     * 选择最具体的路径（单一路径选择）
     * 
     * <p>快速失败设计：当候选列表为空或 {@code null} 时抛出 {@link IllegalArgumentException}，
     * 而非返回 {@code null}。调用方应保证传入至少一个候选路径。</p>
     * 
     * @param candidates 路径候选列表，不得为 {@code null} 或空
     * @return 最具体的路径候选（never null）
     * @throws IllegalArgumentException 如果 {@code candidates} 为 {@code null} 或空
     */
